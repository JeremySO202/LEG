...
class O:
    ...
    def execute(self):
        print(f"Aplicando O a valores")
        if self.bovedareg1 or self.bovedareg2:
            print("Usando registros de boveda")
        
        tempA, tempB = self._get_operand_values()
        self.procesador.regALU.data = self.procesador.ALU.operar(tempA, tempB, 3)
        print(f"Resultado ALU: {self.procesador.regALU.data}")
    
    def memory(self):
        print(f"Sin operación de memoria para O")
    ...
    def ejecutar(self):
        if self.ejecucion:
            ...
            print("No hay más fases para ejecutar en O.")
