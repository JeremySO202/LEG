# Documentación de Diseño del Proyecto
---

## **Información del Proyecto**

- **Nombre del Proyecto**: LEG - Microarquitectura y Diseño de Procesadores

- **Equipo**: Equipo LEG

- **Integrantes**:
  - Integrante 1: Andres Blanco Coto
  - Integrante 2: Karina Martínez Guerrero 
  - Integrante 3: Fabiola Meléndez Sequeira
  - Integrante 4: Jeremy Noryan Serracin Oporta
- **Profesor**: Jeferson Gonzalez Gomez
- **Materia**: Arquitectura de Computadores
- **Fecha**: 10/10/2025


# Índice

1. [Green Card](./Documentation%20files/ISA_reference_v1.0.md)
2. [Arquitectura del set de instrucciones](./Documentation%20files/ISA_decisions.md)
3. [Microarquitectura](./Documentation%20files/Organization_v0.1.md)
4. [Modelado del software](./Documentation%20files/Software_Model.md)
5. [Referencias](./Documentation%20files/Referencias.md)
